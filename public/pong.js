...
const END_GAME_DELAY = 4000;
const POINTS_FOR_VICTORY = 5;
const BALL_INITIAL_SPEED = 9;
const BALL_HORIZONTAL_SPEED = 1;      // relative horizontal/vertical components
const BALL_VERTICAL_SPEED = 1;        // of speed; affects deflection angles (tweak to prevent endless "no move necessary" games)
const AI_SPEED_DEFENSIVE = 0.66;      // affects difficulty!
const AI_SPEED_OFFENSIVE = 0.20;      // slightly affects difficulty!
const AI_SPEED_SERVE = 0.80;          // put higher than AI_SPEED_DEFENSIVE to reduce aces against AI, which are unsatisfying
const VOLLEY_BEFORE_ACCELERATION = 4; // how many bounces before the ball starts to accelerate
const VOLLEY_ACCELERATION = 0.25;     // how much the ball accelerates per subsequent bounce

// Gamepads (JS Gamepad API)
let controllers = [];
...
const Game = {
  ...
  menu: function () {
    ...
    this.context.fillText('How many players? 1 or 2',
      this.canvas.width / 2,
  ...
  update: function () {
    scangamepads(); // update status of gamepads
    ...
    if (!this.over) {
      ...
      // Check state of first axis on odd-numbered gamepads and update player1.move, if applicable
      // (overriding keyboard commands set by event handler; gamepad state is polled, not evented)
      for(let i = 0; i < controllers.length; i+=2){
        const controller = controllers[i];
        if(controller){
          if(controller.axes[1] < -0.5) { this.player1.move = DIRECTION.UP; this.player1.movedByController = true; }
          if(controller.axes[1] >  0.5) { this.player1.move = DIRECTION.DOWN; this.player1.movedByController = true; }
        }
      }

      // Unless AI'd, state of first axis on even-numbered gamepads and update player2.move, if applicable
      // (overriding keyboard commands set by event handler; gamepad state is polled, not evented)
      if(!this.player2.ai){
        for(let i = 1; i < controllers.length; i+=2){
          const controller = controllers[i];
          if(controller){
            if(controller.axes[1] < -0.5) { this.player2.move = DIRECTION.UP; this.player2.movedByController = true; }
            if(controller.axes[1] >  0.5) { this.player2.move = DIRECTION.DOWN; this.player2.movedByController = true; }
          }
        }
      }

      // Move player 1 if they player1.move value was updated by a keyboard event
      ...
      // Move player 2 if they're not an AI and player2.move value was updated by a keyboard event
      if(!this.player2.ai){
      ...
      // If player 2 collides with the bound limits and is not an AI, update the x and y coords.
      if(!this.player2.ai){
      ...
      if (this.ball.moveY === DIRECTION.UP) this.ball.y -= (this.ball.speed * BALL_HORIZONTAL_SPEED);
      else if (this.ball.moveY === DIRECTION.DOWN) this.ball.y += (this.ball.speed * BALL_HORIZONTAL_SPEED);
      if (this.ball.moveX === DIRECTION.LEFT) this.ball.x -= (this.ball.speed * BALL_VERTICAL_SPEED);
      else if (this.ball.moveX === DIRECTION.RIGHT) this.ball.x += (this.ball.speed * BALL_VERTICAL_SPEED);

      // AI
      if(this.player2.ai){
        // Handle player 2 AI UP and DOWN movement
        if (this.player2.y > this.ball.y - (this.player2.height / 2)) {
          this.player2.y -= this.player2.speed * AI_SPEED_OFFENSIVE;
          if (this.ball.moveX === DIRECTION.RIGHT) this.player2.y -= this.player2.speed * AI_SPEED_DEFENSIVE;
          if (this.ball.volley === 0) this.player2.y -= this.player2.speed * AI_SPEED_SERVE;
        }
        if (this.player2.y < this.ball.y - (this.player2.height / 2)) {
          this.player2.y += this.player2.speed * AI_SPEED_OFFENSIVE;
          if (this.ball.moveX === DIRECTION.RIGHT) this.player2.y += this.player2.speed * AI_SPEED_DEFENSIVE;
      ...
      // In extended volleys, accelerate the ball
      if(this.ball.volley > VOLLEY_BEFORE_ACCELERATION){
        this.ball.speed = BALL_INITIAL_SPEED + ((this.ball.volley - VOLLEY_BEFORE_ACCELERATION) * VOLLEY_ACCELERATION);
      }

      // If player was moved using a controller, set movement to idle (and re-poll next loop) because gamepads are polled, not evented
      if(this.player1.movedByController){ this.player1.movedByController = false; this.player1.move = DIRECTION.IDLE }
      if(this.player2.movedByController){ this.player2.movedByController = false; this.player2.move = DIRECTION.IDLE }
    }

    // Handle the end of round transition
    // Check to see if the player won the round.
    if (this.player1.score >= POINTS_FOR_VICTORY) {
      this.over = true;
      setTimeout(function () { Pong.endGameMenu('Player 1 wins!'); }, TURN_DELAY);
    } else if (this.player2.score >= POINTS_FOR_VICTORY) {
      this.over = true;
      setTimeout(function () { Pong.endGameMenu('Player 2 wins!'); }, TURN_DELAY);
    }
  },
  ...
  listen: function () {
    document.addEventListener('keydown', function (key) {
      ...
      if(Pong.running === false) {
        if(key.keyCode != 49 && key.keyCode != 50) return; // only listening for 1/2
        Pong.running = true;
        Pong.player2.ai = (key.keyCode == 49); // P1 vs AI game or P1 vs P2 game?
      ...
      if(!Pong.player2.ai){
        // Pressed up arrow (P2 up)
        if (key.keyCode === 38) Pong.player2.move = DIRECTION.UP;
        // Pressed down arrow (P2 down)
    ...
    document.addEventListener('keyup', function (key) {
      // Released W (P1 up) or S (P1 down)
      if (key.keyCode === 87 || key.keyCode === 83) Pong.player1.move = DIRECTION.IDLE;

      if(!Pong.player2.ai){
        // Released up arrow (P2 up) or down arrow (P2 down)
  ...
  _turnDelayIsOver: function() {
    return ((new Date()).getTime() - this.timer >= TURN_DELAY);
  }
};
